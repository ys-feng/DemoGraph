...
class GNNActorTrainer(Trainer):
    def __init__(self, config: OrderedDict):
        ...
        # Load Actor graph, labels, and splits
        ...
        if self.cga_config.get("use_cga", False):
            self.augment_graph("/home/r10user16/GraphAug/txts/datasetlevel_actor_25_triples.txt",
                               "/home/r10user16/GraphAug/txts/datasetlevel_actor_25_triples.txt")
    ...
    def train(self):
        ...
        for epoch in training_range:
            ...
            # Metrics
            train_acc = calculate_accuracy(preds[torch.cat([self.train_mask[:, 0],
                                                            torch.zeros(self.n_new_nodes,
                                                                        device=self.device).type_as(
                                                                self.train_mask)])
                                           ], labels)
            # train_acc = calculate_accuracy(preds[self.train_mask[:, 0]], labels)
            ...
            wandb.log({"Epoch": epoch + 1, "Train Loss": loss.item(), "Train ACC": train_acc, "Test ACC": test_acc, "Test F1": test_f1, "Test AUROC": auroc})
            # ,
    ...
    def evaluate(self):
        ...
        labels = self.labels[self.test_mask[:, 0]].to(self.device)

        # test_acc = calculate_accuracy(preds[self.test_mask[:, 0]], labels)
        preds = preds[torch.cat([self.test_mask[:, 0],
                                 torch.zeros(self.n_new_nodes, device=self.device).type_as(self.test_mask)])
        ]

        # preds = preds[[self.test_mask[:, 0]]]
