...
#For type-level approach:
    def read_new_concepts(self):
        new_concepts = []
        with open('/home/r10user16/GraphAug/txts/typelevel_procedure.txt', 'r') as f:
            for line in f.readlines():
                new_concepts.append(line.strip())
        return new_concepts

    import random

    def add_new_concepts_to_edge_dict(self, new_concepts):
        new_edge_dict = self.edge_dict.copy()
        for key in self.edge_dict.keys():
            visit_nodes, procedure_nodes = self.edge_dict[key]  # Assuming these are the correct names

            if key == ('visit', 'treated', 'procedure'):
                random_visit_nodes = random.choices(visit_nodes, k=min(200, len(visit_nodes)))
                random_new_concepts = random.choices(new_concepts, k=200)

                for visit, new_concept in zip(random_visit_nodes, random_new_concepts):
                    procedure_nodes.append(new_concept)

                new_edge_dict[key] = (visit_nodes, procedure_nodes)
            else:
                new_edge_dict[key] = (visit_nodes, procedure_nodes)

        # Create the DGL graph here
        merged_graph = dgl.heterograph(new_edge_dict)

        # Initialize node features if needed
        feat_dim = 128  # Example feature dimension
        for ntype in merged_graph.ntypes:
            n_nodes = merged_graph.num_nodes(ntype)
            # Initialize features
            feat = torch.randn(n_nodes, feat_dim)
            merged_graph.nodes[ntype].data['feat'] = feat
    ...
    def train(self) -> None:
        ...
        # Read new concepts from file
        new_concepts = self.read_new_concepts()

        # Add new concepts to edge_dict to create new_edge_dict
        new_edge_dict = self.add_new_concepts_to_edge_dict(new_concepts)

        # Update self.edge_dict
        ...
        for epoch in training_range:
            ...
            self.graph = self.add_new_concepts_to_edge_dict(self, new_concepts)
            ...
            training_range.set_description_str(
                "Epoch {} | loss: {:.4f}| Train AUC: {:.4f} | Test AUC: {:.4f} | Test ACC: {:.4f} ".format(
                    epoch, loss.item(), train_metrics["tr_accuracy"], test_metrics["test_accuracy"],
    ...
    def load_kg(entity):
       ...
       file_path = f'./txts/full_D_ICD_PROCEDURES/{entity}.txt'
       df = None  # Initialize df to None
       with open(file_path, 'r') as file:
           try:
               df = pd.read_csv(file, delimiter='\t',
                                header=None)  # Set header=None for DataFrames without column names
           except pd.errors.ParserError as e:
               print(f"ParserError in file: {file_path}")
               # (Your existing code for handling ParserError)
    ...
    def get_augmentation_entities(self):
       ...
       # return self.graph.nodes('cond').tolist()
