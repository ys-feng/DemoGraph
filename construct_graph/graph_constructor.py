...
class GraphConstructor:
    def __init__(self, config_graph):
        ...
        if not Path(self.graph_path).exists():
            Path(self.graph_path).mkdir(parents=True)

        if not Path(self.cache_path).exists():
            Path(self.cache_path).mkdir(parents=True)
    ...
    # below is newly added, trying to fix FileNotFoundError: [Errno 2] No such file or directory: './data/graphs/mimic3_dp.pkl'
    def load_graph(self):
        ...
        try:
            with open(graph_path, 'rb') as inp:
                unp = pickle.Unpickler(inp)           # 读取文件的逻辑
                g = unp.load()
