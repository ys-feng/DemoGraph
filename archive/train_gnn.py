...
class GNNTrainer(Trainer):
    ...
    def train(self) -> None:
       ...
       for epoch in training_range:
           ...
           #NEW: Update all labels in self.graph into string-type
           ...
           for t in self.tasks:
               ...
               names = self.index_to_names(indices)

               d["visit"] = self.node_dict["visit"][indices]
               sg = self.graph.subgraph(d).to(self.device)
               preds = self.gnn(sg, "visit", t)

               labels = torch.LongTensor([self.labels[t][i] for i in names]).to(self.device)
    ...
    def merge_ehr_kg(self, kg_df, ehr_entity_names):
        ...
        for edge in kg_edges:
            ...
            concept_head = entity_mapping[edge['procedure']]
            concept_tail = entity_mapping[edge['concept']]
        ...
        for concept in llm_concepts:
            mapped_concept = entity_mapping[concept]
