                ...
                for lr in [
                    ...
                    for heads in [
                        ...
                        for nec in [
                            ...
                            config["optimizer"]["lr"] = lr
                            config["GNN"]["nec"] = nec
                            config["name"] = f"CORA_CGA_GAT_d{d}dim{dim}L{l}h{heads}lr{lr}nec{nec}tm"
                            dataset_name = config["datasets"]["name"]
                            config["checkpoint"]["path"] = f"./checkpoints/HParameters{dataset_name}_CORA_CGA_GAT/d{d}dim{dim}L{l}h{heads}lr{lr}nec{nec}tm/"
...
#For GraphSAGE:
    ...
    #                     config["optimizer"]["lr"] = lr
    #                     config["GNN"]["nec"] = nec
    #                     config["name"] = f"CORA_CGA_SAGE_d{d}dim{dim}L{l}lr{lr}nec{nec}"
    #                     dataset_name = config["datasets"]["name"]
    #                     config["checkpoint"][
    #                         "path"] = f"./checkpoints/HParameters{dataset_name}_CORA_CGA_SAGE/d{d}dim{dim}L{l}lr{lr}nec{nec}/"
    #
    #                     config["logging"]["tags"] += ["hp"]
    #
    #                     trainer = GNNCoraTrainer(config)
...
# For GCN:
    ...
    #                     config["optimizer"]["lr"] = lr
    #                     config["GNN"]["nec"] = nec
    #                     config["name"] = f"CORA_CGA_GCN_d{d}dim{dim}L{l}lr{lr}nec{nec}"
    #                     dataset_name = config["datasets"]["name"]
    #                     config["checkpoint"][
    #                         "path"] = f"./checkpoints/HParameters{dataset_name}_CORA_CGA_GCN/d{d}dim{dim}L{l}lr{lr}nec{nec}/"
    #                     config["logging"]["tags"] += ["hp"]
    #
    #                     trainer = GNNCoraTrainer(config)
    #                     trainer.train()
            ...
            for l in [
                ...
                for lr in [
                    ...
                    for nec in [
                        ...
                        config["optimizer"]["lr"] = lr
                        config["GNN"]["nec"] = nec
                        config["name"] = f"CORA_CGA_GIN_d{d}dim{dim}L{l}lr{lr}nec{nec}dyn"
                        dataset_name = config["datasets"]["name"]
                        config["checkpoint"][
                            "path"] = f"./checkpoints/HParameters{dataset_name}_CORA_CGA_GIN/d{d}dim{dim}L{l}lr{lr}nec{nec}dyn/"
                        config["logging"]["tags"] += ["hp"]
